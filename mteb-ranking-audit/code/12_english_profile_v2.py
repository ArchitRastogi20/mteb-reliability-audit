...
Data sources:
  mteb_results_repo/results/<org__model>/<rev>/<task>.json
  (requires pre-cloned results repo from the main download pipeline)
"""
...
RESULTS  = DATA / "mteb_results_repo" / "results"
...
# granite-97m-r2 appears only in the extended tier; added here for the cluster analysis.
EXTRA_CLUSTER = {
    "granite-97m-r2": "ibm-granite/granite-embedding-97m-multilingual-r2",
}

# Five models identified as hidden-failure cluster (see hidden_failure_cluster.csv).
CLUSTER_NAMES = {"granite-311m", "harrier-0.6b", "Seed1.6-embed", "inf-retriever-v1", "granite-97m-r2"}

# Canonical display order for the LaTeX table rows.
...
def find_model_dir(hf_id: str) -> Path | None:
    ...
    if len(candidates) == 1:
        return candidates[0]
    # Prefer exact HF-style match (org__model)
...
def write_latex_table(profile: pd.DataFrame) -> None:
    ...
    for name in CLUSTER_ORDER:
        ...
        lines.append(f"{name} & {pct(r['mean_eng'])} & {pct(r['mean_multi'])} & {sign}{pct(r['delta'])} \\\\")
...
def write_mechanism_text(profile: pd.DataFrame, mechanism_holds: bool) -> None:
    ...
    if mechanism_holds:
        # All cluster models have positive delta.
        ...
        # Mixed results: only harrier-0.6b shows clear English-overweighting.
        # The more general mechanism is averaging-induced masking.
        positive_cl = cluster[cluster["delta"] > 0]["display_name"].tolist()
        tex = (
...
def main():
    if not RESULTS.exists():
        print(f"ERROR: results repo not found at {RESULTS}\n"
              "Run the main download pipeline first.", file=sys.stderr)
        sys.exit(1)
    ...
    # Acceptance check: test whether all cluster models have positive delta.
    ...
    # Secondary check: cluster distinctiveness vs non-cluster.
