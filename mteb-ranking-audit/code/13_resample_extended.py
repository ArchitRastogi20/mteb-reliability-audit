...
and recompute Kendall τ. If resampled τ ≈ 0.69, the jump is mechanical (sample
size). If resampled τ ≈ 0.34, it is driven by which models are in the extended
tier (selection bias).
...
Data sources (pre-computed by the main pipeline):
...
def load_extended(iso3: str, subdir: str) -> pd.DataFrame | None:
    ...
    global_path = DATA / "global" / "mteb_agg_extended.csv"
...
def tau_and_inv(global_scores: np.ndarray, local_scores: np.ndarray) -> tuple[float, float]:
    ...
    # Count pairs where global order and local order disagree.
...
def interpret(median_tau: float) -> str:
    ...
    The test: draw 25 random models from the extended pool. Two hypotheses:

    H1 (sample_size_artifact): extended tau is high just because n=57 > 25.
      Evidence: resampled tau at n=25 falls close to the curated level (0.34).
      Implication: the curated-extended gap is a sample-size artifact; both
      pools have the same underlying tau ~ 0.34.

    H2 (selection_bias_in_extended): extended tau is high because ALL extended
      models are multilingual-first (self-selected into MMTEB). Even 25 random
      extended models preserve the high tau.
      Evidence: resampled tau at n=25 stays close to the extended level (0.69).
      Implication: the composition of the extended tier inflates tau; the
      curated tier's lower tau is real and driven by its English-heavy additions.
...
def write_paragraph(summary: dict) -> None:
    ...
    if np.isnan(med):
        ...
        # Resampled tau stays close to the extended level even at n=25.
        # This confirms that extended's high tau is driven by model composition
        # (multilingual-first self-selection), not by having more models.
        paragraph = (
        ...
        # Resampled tau falls close to the curated level, suggesting the true
        # tau is ~0.34 and extended's high tau is a sample-size artifact.
        paragraph = (
            f"\\paragraph{{Roster size vs.\\ selection bias.}}\n"
...
def main():
    ...
    (ANALYSIS / "resample_summary.json").write_text(json.dumps(summary, indent=2), encoding="utf-8")
