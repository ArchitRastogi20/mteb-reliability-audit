...
from scripts.utils import disk_usage_bytes, DISK_BUDGET_BYTES
...
class ModelPipeline:
    ...
    Downloads model[i+1] in background while model[i] is being evaluated.
    Deletes each model's files from disk after the caller finishes with it,
    unless keep_models=True.
    """
    ...
    def _download_with_quota(self, model_id: str) -> Path:
        while disk_usage_bytes(self.cache_dir.parent) > self.disk_budget:
            print(
                ...
                f"({disk_usage_bytes(self.cache_dir.parent)/1e9:.1f}GB used). Waiting…"
            )
            time.sleep(30)
    ...
    def _cleanup(self, local_path: Path) -> None:
        """Delete the entire HF cache entry for this model."""
        try:
